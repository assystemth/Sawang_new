               ...
               <div class="popup-content">
                  ...
                  <form action="<?php echo site_url('intra_share_file/add_intra_sf_council'); ?>" method="post" class="form-horizontal" enctype="multipart/form-data">
                     ...
                     <div class="form-group row container">
                        ...
                        <div class="col-sm-6">
                           <input type="file" name="intra_sf_council_pdf" class="form-control" accept=".pdf, .xls, .xlsx, .doc, .docx, .ppt, .pptx" required>
               ...
               ?>
                  <tr>
                     <td class="font-20 limit-font-one " style="padding-left: 30px;">
                        <?php if (strpos($rs->intra_sf_council_pdf, '.pdf') !== false) : ?>
                           <!-- ถ้ามี .pdf อยู่ในชื่อไฟล์ -->
                           <a class="underline black" href="<?= site_url('intra_share_file/intra_sf_council_detail/' . $rs->intra_sf_council_id); ?>">
                              <img src="<?php echo base_url($iconImage); ?>" width="30"> &nbsp;<?= $rs->intra_sf_council_name; ?>
                           </a>
                        <?php else : ?>
                           <!-- ถ้าไม่มี .pdf อยู่ในชื่อไฟล์ -->
                           <a class="underline black" onclick="downloadFile(event, <?= $rs->intra_sf_council_id; ?>)" href="<?= base_url('docs/intranet/file/' . $rs->intra_sf_council_pdf); ?>" download>
                              <img src="<?php echo base_url($iconImage); ?>" width="30"> &nbsp;<?= $rs->intra_sf_council_name; ?>
                           </a>
                           <script>
                              function downloadFile(event, intra_sf_council_id) {
                                 // ทำการส่งคำร้องขอ AJAX ไปยัง URL ที่บันทึกการดาวน์โหลดพร้อมกับ ID
                                 var xhr = new XMLHttpRequest();
                                 xhr.open('GET', '<?= base_url('intra_share_file/increment_download/'); ?>' + intra_sf_council_id, true);
                                 xhr.send();
                              }
                           </script>
                        <?php endif; ?>
                              ...
                              function confirmDelete(intra_sf_council_id) {
                                 Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                       window.location.href = "<?= site_url('Intra_share_file/del_intra_sf_council/'); ?>" + intra_sf_council_id;
                                    }
                                 });
                              }
                           </script>
                     ...
                     <td class="font-20" align="center">
                        <a onclick="downloadFile(event, <?= $rs->intra_sf_council_id; ?>)" href="<?= base_url('docs/intranet/file/' . $rs->intra_sf_council_pdf); ?>" download>
                        ...
                        <script>
                           function downloadFile(event, intra_sf_council_id) {
                              // ทำการส่งคำร้องขอ AJAX ไปยัง URL ที่บันทึกการดาวน์โหลดพร้อมกับ ID
                              var xhr = new XMLHttpRequest();
                              xhr.open('GET', '<?= base_url('Intra_share_file/increment_download_intra_sf_council/'); ?>' + intra_sf_council_id, true);
                              xhr.send();
                           }
                        </script>
                     </td>
