<div class="text-center pages-head">
    <span class="font-pages-head">รายงานใช้จ่ายงบประมาณจัดซื้อจัดจ้าง</span>
</div>
</div>
...
<div class="bg-pages">
    <div class="container-pages-detail">
        ...
        <div class="pages-content break-word mt-2">
            ...
            <br>
            <?php 
                if($rsData->p_reb_link != "") {
                    echo '<span class="font-pages-content-detail">ลิ้งค์เพิ่มเติม:</span>&nbsp;<a class="font-26" href="' . $rsData->p_reb_link . '" target="_blank">' . $rsData->p_reb_link . '</a>';
                }
            ?>
            <br>
            <?php if (!empty($rsDoc)) { ?>
                ...
                <?php foreach ($rsDoc as $doc) {
                    // ดึงข้อมูลของไฟล์
                    $fileInfo = pathinfo($doc->p_reb_file_doc);
                ...
                ?>
                 <br><img src="<?php echo base_url($iconImage); ?>" style="padding: 0 30px;">
                    <a class="font-doc" href="<?= base_url('docs/file/' . $doc->p_reb_file_doc); ?>" target="_blank"><?= $doc->p_reb_file_doc; ?></a> , &nbsp;
                <?php } ?>
            <?php } ?>
        </div>
        ...
        <?php foreach ($rsImg as $img) { ?>
            <div class="text-center">
                <img class="rounded-all" src="<?php echo base_url('docs/img/' . $img->p_reb_img_img); ?>" width="1035" height="100%">
            </div>
            <br>
        <?php } ?>
        ...
        <?php foreach ($rsPdf as $file) { ?>
            <div class="row">
                <div class="col-6 mt-2">
                    <div class="d-flex justify-content-start">
                        <span class="font-page-detail-view-news">ดาวโหลดแล้ว <?= $file->p_reb_pdf_download; ?> ครั้ง</span>
                    </div>
                </div>
                <div class="col-6">
                    <div class="d-flex justify-content-end">
                        <a onclick="downloadFile(event, <?= $file->p_reb_pdf_id; ?>)" href="<?= base_url('docs/file/' . $file->p_reb_pdf_pdf); ?>" download>
                            <img src="<?php echo base_url("docs/btn-download.png"); ?>" class="btn-download" >
                        </a>
                        <script>
                            function downloadFile(event, p_reb_pdf_id) {
                                // ทำการส่งคำร้องขอ AJAX ไปยัง URL ที่บันทึกการดาวน์โหลดพร้อมกับ ID
                                var xhr = new XMLHttpRequest();
                                xhr.open('GET', '<?= base_url('Pages/increment_download_p_reb/'); ?>' + p_reb_pdf_id, true);
                                xhr.send();
            ...
            <div class="blog-text mt-3 mb-5">
                <object data="<?= base_url('docs/file/' . $file->p_reb_pdf_pdf); ?>" type="application/pdf" width="100%" height="1500px"></object>
            </div>
        <?php } ?>
        <div class="d-flex justify-content-start">
