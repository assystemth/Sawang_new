...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-black">จัดการข้อมูลรข่าวประชาสัมพันธ์</h6>
    </div>
    <div class="card-body">
                        ...
                        <tr role="row">
                            ...
                            <td>
                                <?php if (!empty($rs->news_img)) : ?>
                                    <img src="<?php echo base_url('docs/img/' . $rs->news_img); ?>" width="120px" height="80px">
                                <?php else : ?>
                            ...
                            <td>
                                <?php foreach ($rs->pdf as $pdf) : ?>
                                    <a class="btn btn-primary btn-sm mt-1" href="<?php echo base_url('docs/file/' . $pdf->news_pdf_pdf); ?>" target="_blank">ดูไฟล์เดิม!</a>
                                <?php endforeach; ?>
                                <br>
                                <?php foreach ($rs->doc as $doc) : ?>
                                    <a class="btn btn-info btn-sm mt-1" href="<?php echo base_url('docs/file/' . $doc->news_file_doc); ?>" target="_blank">ดูไฟล์เดิม!</a>
                                <?php endforeach; ?>
                            </td>
                            ...
                            <td>
                                <label class="switch">
                                    <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheck<?= $rs->news_id; ?>" data-news-id="<?= $rs->news_id; ?>" <?= $rs->news_status === 'show' ? 'checked' : ''; ?> onchange="update_news_status<?= $rs->news_id; ?>()">
                                    <span class="slider"></span>
                                </label>
                                <script>
                                    function update_news_status<?= $rs->news_id; ?>() {
                                        ...
                                        $.ajax({
                                            type: 'POST',
                                            url: 'news_backend/update_news_status',
                                            data: {
                                                news_id: newsId,
                            ...
                            <td>
                                <a href="<?= site_url('news_backend/editing/' . $rs->news_id); ?>"><i class="bi bi-pencil-square fa-lg "></i></a>
