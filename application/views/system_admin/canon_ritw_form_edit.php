                    ...
                    <div class="col-sm-6">
                        <?php if (!empty($rsPdf)) { ?>
                            <?php foreach ($rsPdf as $pdf) { ?>
                                <a class="btn btn-primary btn-sm mb-2" href="<?= base_url('docs/file/' . $pdf->canon_ritw_pdf_pdf); ?>" target="_blank">ดูไฟล์ <?= $pdf->canon_ritw_pdf_pdf; ?></a>
                                <a class="btn btn-danger btn-sm mb-2" href="#" role="button" onclick="confirmDeletePdf(<?= $pdf->canon_ritw_pdf_id; ?>, '<?= $pdf->canon_ritw_pdf_pdf; ?>');">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
                                        <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z" />
                                    </svg> ลบไฟล์
                                </a>
                        ...
                        <script>
                            function confirmDeletePdf(pdf_id, pdf_name) {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        // หลังจากคลิกยืนยันให้เรียก Controller ที่ใช้ในการลบไฟล์ PDF
                                        window.location.href = "<?= site_url('canon_ritw_backend/del_pdf/'); ?>" + pdf_id;
                                    }
                                });
                            }
                        </script>
                        <input type="file" name="canon_ritw_pdf_pdf[]" class="form-control mt-1" accept="application/pdf" multiple>
                        <span class="black-add">สามารถอัพโหลดได้หลายไฟล์</span>
                        <br>
                    ...
                    <div class="col-sm-6">
                        <?php if (!empty($rsDoc)) { ?>
                            <?php foreach ($rsDoc as $doc) { ?>
                                <a class="btn btn-info btn-sm mb-2" href="<?= base_url('docs/file/' . $doc->canon_ritw_file_doc); ?>" target="_blank">ดูไฟล์ <?= $doc->canon_ritw_file_doc; ?></a>
                                <a class="btn btn-danger btn-sm mb-2" href="#" role="button" onclick="confirmDeleteDoc(<?= $doc->canon_ritw_file_id; ?>, '<?= $doc->canon_ritw_file_doc; ?>');">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
                                        <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z" />
                                    </svg> ลบไฟล์
                                </a>
