    ...
    <div id="popupInsert" class="popup">
        <div class="popup-content">
            ...
            <form method="post" action="<?php echo base_url() ?>Kid_aw_backend/importcsv" enctype="multipart/form-data">
            ...
            <!-- <form action="<?php echo site_url('Kid_aw_backend/add_csv_data_to_database'); ?> " method="post" class="form-horizontal" enctype="multipart/form-data">
    ...
    <a class="btn btn-light" href="<?= site_url('kid_aw_backend'); ?>" role="button">
                    ...
                    <tbody>
                        ...
                        foreach ($query as $rs) { ?>
                            <tr role="row">
                                ...
                                <td><?= $rs->kid_aw_id_num_eligible; ?></td>
                                <td><?= $rs->kid_aw_name_eligible; ?></td>
                                <td><?= $rs->kid_aw_id_num_owner; ?></td>
                                <td><?= $rs->kid_aw_name_owner; ?></td>
                                <td><?= $rs->kid_aw_agency; ?></td>
                                <td><?= $rs->kid_aw_bank; ?></td>
                                <td><?= $rs->kid_aw_type_payment; ?></td>
                                <td><?= $rs->kid_aw_bank_num; ?></td>
                                <td><?= $rs->kid_aw_period_payment; ?></td>
                                <td><?= $rs->kid_aw_money; ?></td>
                                <td><?= $rs->kid_aw_note; ?></td>
                                <td><?= $rs->kid_aw_by; ?></td>
                                <td><?= date('d/m/Y H:i', strtotime($rs->kid_aw_datesave . '+543 years')) ?> น.</td>
                                <td>
                                    <a href="<?= site_url('kid_aw_backend/editing_kid_aw/' . $rs->kid_aw_id); ?>"><i class="bi bi-pencil-square fa-lg "></i></a>
                                    <a href="#" role="button" onclick="confirmDelete('<?= $rs->kid_aw_id; ?>');"><i class="bi bi-trash fa-lg "></i></a>
                                    <script>
                                        function confirmDelete(kid_aw_id) {
                                            Swal.fire({
                                                title: 'กดเพื่อยืนยัน?',
                                                ...
                                                if (result.isConfirmed) {
                                                    window.location.href = "<?= site_url('kid_aw_backend/del_kid_aw/'); ?>" + kid_aw_id;
                                                }
                                            });
                                        }
                                    </script>
