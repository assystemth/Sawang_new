...
class Canon_chh_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        if (isset($_FILES['canon_chh_pdf_pdf'])) {
            foreach ($_FILES['canon_chh_pdf_pdf']['name'] as $index => $name) {
                if (isset($_FILES['canon_chh_pdf_pdf']['size'][$index])) {
                    $total_space_required += $_FILES['canon_chh_pdf_pdf']['size'][$index];
                }
            }
        }

        // ตรวจสอบว่ามีข้อมูลไฟล์ doc หรือไม่
        if (isset($_FILES['canon_chh_file_doc'])) {
            foreach ($_FILES['canon_chh_file_doc']['name'] as $index => $name) {
                if (isset($_FILES['canon_chh_file_doc']['size'][$index])) {
                    $total_space_required += $_FILES['canon_chh_file_doc']['size'][$index];
                }
            }
        }
        ...
        if (!empty($_FILES['canon_chh_pdf_pdf']['name'][0])) {
            foreach ($_FILES['canon_chh_pdf_pdf']['name'] as $index => $name) {
                $_FILES['canon_chh_pdf_pdf_multiple']['name'] = $name;
                $_FILES['canon_chh_pdf_pdf_multiple']['type'] = $_FILES['canon_chh_pdf_pdf']['type'][$index];
                $_FILES['canon_chh_pdf_pdf_multiple']['tmp_name'] = $_FILES['canon_chh_pdf_pdf']['tmp_name'][$index];
                $_FILES['canon_chh_pdf_pdf_multiple']['error'] = $_FILES['canon_chh_pdf_pdf']['error'][$index];
                $_FILES['canon_chh_pdf_pdf_multiple']['size'] = $_FILES['canon_chh_pdf_pdf']['size'][$index];

                if ($this->pdf_upload->do_upload('canon_chh_pdf_pdf_multiple')) {
                    $upload_data = $this->pdf_upload->data();
                    $pdf_data[] = array(
                        'canon_chh_pdf_ref_id' => $canon_chh_id,
                        'canon_chh_pdf_pdf' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_canon_chh_pdf', $pdf_data);
        }

        $doc_data = array();

        // ตรวจสอบว่ามีการอัปโหลดไฟล์Docเพิ่มเติมหรือไม่
        if (!empty($_FILES['canon_chh_file_doc']['name'][0])) {
            foreach ($_FILES['canon_chh_file_doc']['name'] as $index => $name) {
                $_FILES['canon_chh_file_doc_multiple']['name'] = $name;
                $_FILES['canon_chh_file_doc_multiple']['type'] = $_FILES['canon_chh_file_doc']['type'][$index];
                $_FILES['canon_chh_file_doc_multiple']['tmp_name'] = $_FILES['canon_chh_file_doc']['tmp_name'][$index];
                $_FILES['canon_chh_file_doc_multiple']['error'] = $_FILES['canon_chh_file_doc']['error'][$index];
                $_FILES['canon_chh_file_doc_multiple']['size'] = $_FILES['canon_chh_file_doc']['size'][$index];

                if ($this->doc_upload->do_upload('canon_chh_file_doc_multiple')) {
                    $upload_data = $this->doc_upload->data();
                    $doc_data[] = array(
                        'canon_chh_file_ref_id' => $canon_chh_id,
                        'canon_chh_file_doc' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_canon_chh_file', $doc_data);
        }
    ...
    {
        $this->db->select('canon_chh_pdf_pdf');
        $this->db->from('tbl_canon_chh_pdf');
        $this->db->where('canon_chh_pdf_ref_id', $canon_chh_id);
        return $this->db->get()->result();
    }
    public function list_all_doc($canon_chh_id)
    {
        $this->db->select('canon_chh_file_doc');
    ...
    public function read_pdf($canon_chh_id)
    {
        $this->db->where('canon_chh_pdf_ref_id', $canon_chh_id);
        $this->db->order_by('canon_chh_pdf_id', 'DESC');
        $query = $this->db->get('tbl_canon_chh_pdf');
        return $query->result();
    }
    public function read_doc($canon_chh_id)
    {
        $this->db->where('canon_chh_file_ref_id', $canon_chh_id);
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('canon_chh_pdf_pdf');
        $this->db->where('canon_chh_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_canon_chh_pdf');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->canon_chh_pdf_pdf;
        ...
        $this->db->where('canon_chh_pdf_id', $pdf_id);
        $this->db->delete('tbl_canon_chh_pdf');
        $this->space_model->update_server_current();
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('canon_chh_file_doc');
        $this->db->where('canon_chh_file_id', $doc_id);
        $query = $this->db->get('tbl_canon_chh_file');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->canon_chh_file_doc;
        if (file_exists($file_path)) {
            unlink($file_path);
        }
    ...
    public function del_canon_chh_pdf($canon_chh_id)
    {
        ...
        $files = $this->db->get_where('tbl_canon_chh_pdf', array('canon_chh_pdf_ref_id' => $canon_chh_id))->result();

        // ลบ pdf จากตาราง tbl_canon_chh_pdf
        $this->db->where('canon_chh_pdf_ref_id', $canon_chh_id);
        $this->db->delete('tbl_canon_chh_pdf');

        // ลบไฟล์ pdf ที่เกี่ยวข้อง
        foreach ($files as $files) {
            $file_path = './docs/file/' . $files->canon_chh_pdf_pdf;
            if (file_exists($file_path)) {
                unlink($file_path);
            }
        }
    ...
    public function canon_chh_frontend_list()
    {
        $this->db->select('*');
        $this->db->from('tbl_canon_chh');
        $this->db->where('tbl_canon_chh.canon_chh_status', 'show');
