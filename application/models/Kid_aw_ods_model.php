...
class Kid_aw_ods_model extends CI_Model
{
    ...
    public function add_kid_aw_ods()
    {
        ...
        foreach (['kid_aw_ods_file1', 'kid_aw_ods_file2', 'kid_aw_ods_file3'] as $file_key) {
        ...
        if ($used_space_mb + ($total_space_required / (1024 * 1024)) >= $upload_limit_mb) {
            ...
            redirect('Pages/kid_aw_ods');
            return;
        }

        // เตรียมข้อมูลสำหรับการเพิ่มลงในตาราง
        $kid_aw_ods_data = array(
            'kid_aw_ods_by' => $this->input->post('kid_aw_ods_by'),
            'kid_aw_ods_phone' => $this->input->post('kid_aw_ods_phone'),
            'kid_aw_ods_number' => $this->input->post('kid_aw_ods_number'),
            'kid_aw_ods_address' => $this->input->post('kid_aw_ods_address'),
        );
        ...
        foreach (['kid_aw_ods_file1', 'kid_aw_ods_file2', 'kid_aw_ods_file3'] as $file_key) {
            if (isset($_FILES[$file_key]) && !empty($_FILES[$file_key]['name'])) {
                ...
                if (!$this->upload->do_upload($file_key)) {
                    ...
                    $kid_aw_ods_data[$file_key] = $upload_data['file_name'];
        ...
        $this->db->trans_start();
        $this->db->insert('tbl_kid_aw_ods', $kid_aw_ods_data);
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->db->order_by('kid_aw_ods_id', 'DESC');
            $this->db->limit(1);
            $query = $this->db->get('tbl_kid_aw_ods');
            $latest_entry = $query->row();

            // สร้างข้อความแจ้งเตือน
            $message = "เอกสารเงินอุดหนุนเด็กแรกเกิด ใหม่:\n";
            $message .= "ชื่อผู้ยื่นคำร้อง: " . $latest_entry->kid_aw_ods_by . "\n";
            $message .= "โทรศัพท์: " . $latest_entry->kid_aw_ods_phone . "\n";
            $message .= "ที่อยู่: " . $latest_entry->kid_aw_ods_address . "\n";
            // $message .= "เอกสารแนบ: " . $latest_entry->kid_aw_ods_file1 . "\n";
            $message .= "เอกสารแนบ: " . $latest_entry->kid_aw_ods_file2 . "\n";
            // $message .= "เอกสารแนบ: " . $latest_entry->kid_aw_ods_file3 . "\n";
    ...
    public function edit($kid_aw_ods_id)
    {
        $old_document = $this->db->get_where('tbl_kid_aw_ods', array('kid_aw_ods_id' => $kid_aw_ods_id))->row();
        ...
        if ($update_doc_file) {
            ...
            $total_space_required = 0;
            if (!empty($_FILES['kid_aw_ods_file']['name'])) {
                $total_space_required += $_FILES['kid_aw_ods_file']['size'];
            }

            if ($used_space_mb + ($total_space_required / (1024 * 1024 * 1024)) >= $upload_limit_mb) {
                $this->session->set_flashdata('save_error', TRUE);
                redirect('kid_aw_ods/editing');
                return;
            }
            ...
            if (!$this->upload->do_upload('kid_aw_ods_file')) {
            ...
            // ใช้รูปภาพเดิม
            $filename = $old_document->kid_aw_ods_file;
        }

        $data = array(
            'kid_aw_ods_name' => $this->input->post('kid_aw_ods_name'),
            'kid_aw_ods_by' => $this->session->userdata('m_fname'), // เพิ่มชื่อคนที่เพิ่มข้อมูล
            'kid_aw_ods_file' => $filename
        );

        $this->db->where('kid_aw_ods_id', $kid_aw_ods_id);
        $query = $this->db->update('tbl_kid_aw_ods', $data);
    ...
    public function increment_view()
    {
        ...
        $this->db->set('kid_aw_ods_view', 'kid_aw_ods_view + 1', false); // บวกค่า kid_aw_ods_view ทีละ 1
        $this->db->update('tbl_kid_aw_ods');
    }
    public function increment_download_kid_aw_ods()
    {
        $this->db->where('kid_aw_ods_id', 1);
        $this->db->set('kid_aw_ods_download', 'kid_aw_ods_download + 1', false); // บวกค่า kid_aw_ods_view ทีละ 1
        $this->db->update('tbl_kid_aw_ods');
    }
}
